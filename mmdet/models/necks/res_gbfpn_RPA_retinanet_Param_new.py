...
class RGBRARFPN(nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(self.start_level, self.backbone_end_level):
            ...
            # ra = self.make_five_conv([self.out_channels, self.out_channels*2 + self.in_channels[i]], self.out_channels*2 + self.in_channels[i])
            # RA________________________________________________________________________________________________________
            l_conv = ConvModule(
                self.cls_num,
                out_channels,
    ...
    # def make_five_conv(self, filters_list, in_filters):
    #     m = nn.Sequential(
    #         ConvModule(
    #             in_filters, filters_list[0], 1,
    #             conv_cfg=self.conv_cfg,
    #             norm_cfg=self.norm_cfg,
    #             activation=self.activation,
    #             inplace=False),
    #         ConvModule(
    #             filters_list[0], filters_list[1], 3,
    #             padding=1,
    #             conv_cfg=self.conv_cfg,
    #             norm_cfg=self.norm_cfg,
    #             activation=self.activation,
    #             inplace=False),
    #         ConvModule(
    #             filters_list[1], filters_list[0], 1,
    #             conv_cfg=self.conv_cfg,
    #             norm_cfg=self.norm_cfg,
    #             activation=self.activation,
    #             inplace=False),
    ...
    def forward(self, inputs):
        ...
        br_out_lays = [
            br_conv(gcn_out_lays[i]) for i, br_conv in enumerate(self.br_convs)
        ...
        laterals = [
            ...
            lateral_conv(br_out_lays[i]) for i, lateral_conv in enumerate(self.lateral_convs)
        ]
        # 1x1___________________________________________________________________________________________________________
        ...
        #     # print(f"[INFO] inputs[{0}].shape:{inputs[0].shape}")
        #     # print(f"[INFO] inputs[{1}].shape:{inputs[1].shape}")
        #     # print(f"[INFO] inputs[{2}].shape:{inputs[2].shape}")
        #     # print(f"[INFO] inputs[{i + self.start_level}].shape:{inputs[i + self.start_level].shape}")
        #     # print(f"[INFO] temp.shape:{temp.shape}")
        #     laterals[i - 1] = torch.cat([laterals[i - 1], temp, inputs[i - 1 + self.start_level]], axis=1)
        #     # print(f"[INFO] laterals[{i - 1}].shape:{laterals[i - 1].shape}")
